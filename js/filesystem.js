...
function FileSystem($parent, $startingDir) {
    ...
    this.refresh = function() {
        ...
        for (i = 0; i < dir_history.length; i++) {
            ...
            if (dir_name == 'Home') {
                dir_name = '<span class="home"></span>' + dir_name
            }
            if ($.trim(dir_name) == 'Desktop') {
                dir_name = '<img src="../img/folder/desktop-icon.png"/>' + dir_name
            }
            if (i == history_index) {
    ...
    this.backButton = function($enable) {
        if ($enable) {
            if ($('.buttons .folder-back').hasClass('disabled')) {
                $('.buttons .folder-back').bind('click', function() {
                    _this.backDir();
                });
    ...
    this.forwardButton = function($enable) {
        if ($enable) {
            if ($('.buttons .folder-forward').hasClass('disabled')) {
                $('.buttons .folder-forward').bind('click', function() {
                    _this.forwardDir();
                });
