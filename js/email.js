...
function Email($id, $folder, $subject, $body, $from, $to, $date, $cc) {
    ...
    if ($from == undefined) {
        $from = 'Welcome <welcome@ubuntu.com>';
    }
    if ($to == undefined) {
        $to = 'Me <me@canonical.com>';
    }
    if ($cc == undefined) {
        $cc = false;
    }
    if ($body == undefined) {
        $body = 'There is no body for message ' + $subject;
    }
    if ($date == undefined) {
    ...
    this.forwarded = function() {
        return _forwarded;
    }

    this.setReplied = function($replied) {
        _replied = $replied;
    }
    this.setAttachment = function($attachment) {
        _attachment = $attachment;
    }
    this.setStarred = function($starred) {
        _starred = $starred;
    }
    ...
    this.setFire = function($fire) {
        _fire = $fire;
    }
    this.setDeleted = function($deleted) {
        _deleted = $deleted;
    }
    this.setForwarded = function($forwarded) {
